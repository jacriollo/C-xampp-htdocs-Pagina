...
class Secretaria{
    ...
    public static function consultar(){        
        ...
        $sql=$conexionBD->query("SELECT * FROM persona WHERE tip_usu='4'");        
        foreach($sql->fetchAll() as $secretaria) {
            $listausuarios[]= new Secretaria ($secretaria['id_per'],$secretaria['ced_per'],$secretaria['ape_per'],$secretaria['nom_per'],$secretaria['correo_per'],$secretaria['tel_per'],$secretaria['dir_per'],$secretaria['ciu_rec_per'],$secretaria['fec_nac_per'],$secretaria['gen_per'],$secretaria['tip_usu'],$secretaria['est_per'],$secretaria['pas_per']); 
    ...
    public static function crear($cedula,$apellido,$nombre,$correo,$telefono,$direccion,$ciudad,$fecha_nacimiento,$genero,$tipusu,$estper,$pas_per){
        ...
        if($id > 0){
            ...
            Secretaria::crearusuarios($id, $cedula, $pas_per, $tipusu, $estper);
           }
    }
    ...
    public static function buscar($id_per){
        ...
        $sql->execute(array($id_per));
        $usuarios=$sql->fetch();
        return new Secretaria($usuarios['id_per'],$usuarios['ced_per'],$usuarios['ape_per'],$usuarios['nom_per'],$usuarios['correo_per'],$usuarios['tel_per'],$usuarios['dir_per'],$usuarios['ciu_rec_per'],$usuarios['fec_nac_per'],$usuarios['gen_per'],$usuarios['tip_usu'],$usuarios['est_per'],$usuarios['pas_per']);
    }
    ...
    public static function editar($id_per, $cedula,$apellido,$nombre,$correo,$telefono,$direccion,$ciudad,$fecha_nacimiento,$genero,$tipusu,$estper,$pas){
       ...
       // if($id > 0){            
        Secretaria::modificarusuarios($id_per, $cedula, $pas, $tipusu, $estper);
       // }
